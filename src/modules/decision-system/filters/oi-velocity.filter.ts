...
export class OIVelocityFilter {
  ...
  evaluate(symbol: string, data: MarketDataAccessor, rule: { percent: number; oiIntervalMin: number }, windowInfo?: {start?: number, end?: number}): VelocityResult {
    const minutes = rule.oiIntervalMin;
    const now = Date.now();
    const from = windowInfo?.start ?? (now - minutes * 60_000);
    const to = windowInfo?.end ?? now;
    
    // ИСПРАВЛЕННАЯ СТРОКА - используем точный диапазон
    const series: OIPoint[] = data.getOISeriesInRange 
      ? data.getOISeriesInRange(symbol, from, to)
      : data.getOISeries(symbol, Math.ceil((to - from) / 60000)); // fallback
